...
class Article(object):
    ...
    def __init__(self,data,source=None,context=None,http=None):
        
        if isinstance(data,feedparser.FeedParserDict): #if all feed objects
            self.date = try_get_key(data,'published_parsed')
            self.url = try_get_key(data,'link')
            self.authors = try_get_key(data,'author')
            self.title = try_get_key(data,'title')

        elif isinstance(data,str): #if url passed
            self.url = data

        elif all([isinstance(a,str) for a in data]): #if raw paras passed
            self.paras = data
        else:
            raise TypeError('Either enter a feed object, a url, or paragraphs')
        ...
        if context != None:
            if isinstance(context,dict):
                ...
                raise TypeError('Enter a dictionary mapping class variables to values')
    ...
    def text(self,sep='\n'):
            return sep.join(self.paras)+sep #add sep on the end to better support using matching to find paragraphs
    ...
    def get_ents(self,nlp,resolve=False):
        '''Return named ents and resolutions if resolve is set to true'''
        matcher = Matcher(nlp.vocab)
        accepted = ['PERSON','NORP','ORG','GPE','PRODUCT','EVENT','LAW']

        #use new para split token for easy matching to rebuild paragraphs
        new_para_split_token = ' //////// '
        pattern=[{"TEXT":new_para_split_token.strip()}]
        ...
        if resolve==False:
            ...
            ents = []
            if self.coref_setup==False:
                import neuralcoref
                neuralcoref.add_to_pipe(nlp)
                self.coref_setup=True
            clusters=[(ent._.coref_cluster,ent) for ent in doc.ents if ent.label_ in accepted]
            for cluster,ent in clusters: #get ents and their resolutions
                if cluster == None:
                    ents+=[(ent,ent,ent.start,ent.end)]
                    continue
                ents += [(cluster.main,span,span.start,span.end) for span in cluster]
            ents = sorted(ents,key= lambda x: x[2])
            matcher.add('para_break',None,pattern) #find matches for para break
            matches=matcher(doc)


            P_ents=[ [] for i in range(len(self.paras))] #make empty list to hold ents
            count=0
            ranges=[]
            for i,match in enumerate(matches): #make list of ranges of each paragraph
                if i==0:
                    start=0
                end=match[2]
            ...
            ranges = iter(ranges)
            range_=next(ranges) #iterate through list of ranges to place ents in correct range
            for ent in ents:
                resolved,original,start,end = ent
                if start in range_:
                    P_ents[count].append('{} (references {})'.format(original,resolved))
                else:
                    range_=next(ranges)
                    count+=1
            self.ents =  P_ents
...
class Collection(object):
    ...
    def __init__(self,args=None,max_threads=30,source=None,http=None,verbose=1):
        if http is None:
        ...
        if args == None: #if called empty
            self.articles = []
        elif all([isinstance(arg,Article) for arg in args]): #if all articles
            self.articles = list(args)

        elif all([isinstance(arg,feedparser.FeedParserDict) for arg in args]):# if all feed objects
            self.articles = self.thread(args,max_threads)

        elif all([isinstance(arg,str) for arg in args]): #if all urls
            self.articles = self.thread(args,max_threads,source=source)
    ...
    def updater(self,old_collection):
        self.articles = [Article(a.paras,context=a.get_context()) for a in old_collection]
        return self

    def get_ents(self,nlp,resolve=False):
        for article in self.articles:
            article.get_ents(nlp=nlp,resolve=resolve)
        return self
    ...
    def load(file):
        with open(file,'rb') as f:
            c=pickle.load(f)
        try:
            new_c = Collection(c.articles)
        except TypeError: #if different version is detected, rebuild using context
            new_c = Collection().updater(c)
        return new_c
